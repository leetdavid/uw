...
TabOrganizationController::TabOrganizationController(BrowserWindowInterface* window)
    : window_(window), tabs_(window->GetTabStripModel()),
...
TabOrganizationController::~TabOrganizationController() = default;

TabOrganizationController* TabOrganizationController::From(BrowserWindowInterface* window) {
  return window ? Get(window->GetUnownedUserDataHost()) : nullptr;
}

TabOrganizationController* TabOrganizationController::From(TabStripModel* model) {
...
bool TabOrganizationController::Eligible(tabs::TabInterface* tab) const {
  if (!tab || tabs_->GetIndexOfTab(tab) == TabStripModel::kNoTab) return false;
...
std::optional<TreeId> TabOrganizationController::IdFor(const tabs::TabInterface* tab) const {
...
tabs::TabInterface* TabOrganizationController::TabFor(const TreeId& id) const {
  for (const auto& [handle, page] : pages_) {
    if (page == id && Eligible(handle.Get())) return handle.Get();
  }
  return nullptr;
}
...
void TabOrganizationController::Sync() {
  ...
  for (int i = 0; i < tabs_->count(); ++i) {
    ...
    if (Eligible(tab)) live.insert(tab->GetHandle());
  }
  ...
  for (int i = 0; i < tabs_->count(); ++i) {
    ...
    if (auto id = IdFor(tab)) {
      tree_.UpdatePageTitle(*id, tab->GetContents()->GetTitle());
      continue;
    }
    auto* saved = static_cast<PageTreeState*>(
        tab->GetContents()->GetUserData(&kPageTreeStateKey));
    auto id = saved ? tree_.RestorePage(saved->state, tab->GetContents()->GetTitle())
                    : tree_.AddPage(tab->GetContents()->GetTitle());
...
void TabOrganizationController::DidOpenLink(BrowserWindowInterface* window,
  ...
  if (!self || !source || page == source) return;
  self->Sync();
...
void TabOrganizationController::RestoreTab(
    BrowserWindowInterface* window, content::WebContents* page,
    const std::map<std::string, std::string>& extra_data) {
  auto* self = From(window);
  auto it = extra_data.find(kSessionKey);
  if (!self || it == extra_data.end()) return;
  auto state = base::JSONReader::ReadDict(it->second, base::JSON_PARSE_RFC);
  auto* tab = self->tabs_->GetTabForWebContents(page);
  if (!state || !self->Eligible(tab)) return;
  if (auto id = self->IdFor(tab)) self->tree_.RemovePage(*id);
...
std::vector<TreeId> TabOrganizationController::SelectionFor(
  ...
  if (context && tabs_->GetIndexOfTab(context) == TabStripModel::kNoTab) return {};
  if (context && !tabs_->IsTabSelected(tabs_->GetIndexOfTab(context))) {
    if (auto id = IdFor(context)) result.push_back(*id);
    return result;
  }
  for (int index : tabs_->selection_model().GetListSelectionModel().selected_indices()) {
    auto id = IdFor(tabs_->GetTabAtIndex(index));
    if (!id) return {};  // Do not silently act on only part of a mixed selection.
    result.push_back(*id);
  }
...
std::optional<TreeId> TabOrganizationController::MakeTask(
    const std::vector<TreeId>& ids, std::u16string title) {
...
bool TabOrganizationController::Move(const std::vector<TreeId>& ids,
  ...
  if (!tree_.Move(ids, parent, before)) return false;
  AlignTabOrder();
  return true;
}
...
std::vector<tabs::TabInterface*> TabOrganizationController::ResolvePages(
  ...
  for (const auto& id : ids) {
    if (auto* tab = TabFor(id)) pages.push_back(tab);
  }
  return pages;
}
...
void TabOrganizationController::CloseBranch(const std::vector<TreeId>& ids) {
  ...
  if (pages.empty()) {
    for (const auto& id : ids) tree_.RemoveEmptyTask(id);
    return;
  }
  ...
  tabs_->ExecuteCloseTabsCommand(base::BindRepeating(
      ...
      }, weak_factory_.GetWeakPtr(), pages), false);
}
...
void TabOrganizationController::AppendCommands(ui::SimpleMenuModel* menu) {
  ...
  menu->AddItem(kScratchpad, l10n_util::GetStringUTF16(IDS_UW_MOVE_TO_SCRATCHPAD));
  ...
  menu->AddItem(kToggleCollapsed, l10n_util::GetStringUTF16(IDS_UW_TOGGLE_BRANCH));
...
TabOrganizationController::GetMoveTarget(int command, const TreeId& id) const {
  ...
  if (!node || !node->parent()) return std::nullopt;
  ...
  switch (command) {
    case kMoveUp:
      ...
      if (siblings[index - 1]->value.kind == TabTree::Kind::kScratchpad) return std::nullopt;
      break;
    case kMoveDown:
      if (index + 1 >= siblings.size()) return std::nullopt;
      if (index + 2 < siblings.size()) target.before = siblings[index + 2]->value.id;
      break;
    case kNest:
      if (index == 0) return std::nullopt;
    ...
    case kOutdent: {
      ...
      if (!grandparent || parent == tree_.scratchpad()) return std::nullopt;
      // A page leaving a root-level task returns to Scratchpad.
      target.parent = grandparent == tree_.root() && node->value.kind == TabTree::Kind::kPage
          ? tree_.scratchpad()->value.id : grandparent->value.id;
      if (target.parent == grandparent->value.id) {
        const size_t next = grandparent->GetIndexOf(parent).value() + 1;
        if (next < grandparent->children().size()) target.before = grandparent->children()[next]->value.id;
      }
      break;
    }
    default: return std::nullopt;
  }
  return tree_.CanMove({id}, target.parent) ? std::make_optional(target) : std::nullopt;
}

bool TabOrganizationController::CanExecute(int command, const std::vector<TreeId>& ids) const {
  const auto roots = tree_.Normalize(ids);
  if (command == kUndo) return tree_.CanUndo();
  ...
  switch (command) {
    ...
    case kCloseBranch: return true;
    case kScratchpad: return tree_.CanMove(roots, tree_.scratchpad()->value.id);
    case kRenameTask: return roots.size() == 1 && tree_.Find(roots[0])->value.kind == TabTree::Kind::kTask;
    case kToggleCollapsed: return roots.size() == 1 && !tree_.Find(roots[0])->children().empty();
    default: return roots.size() == 1 && GetMoveTarget(command, roots[0]).has_value();
  }
}

std::optional<TreeId> TabOrganizationController::Execute(int command, const std::vector<TreeId>& ids) {
  if (!CanExecute(command, ids)) return std::nullopt;
  const auto roots = tree_.Normalize(ids);
  switch (command) {
    case kNewTask: return MakeTask(roots, l10n_util::GetStringUTF16(IDS_UW_UNTITLED_TASK));
    case kCloseBranch: CloseBranch(roots); return std::nullopt;
    case kUndo: Undo(); break;
    case kScratchpad: Move(roots, tree_.scratchpad()->value.id); break;
    case kToggleCollapsed: tree_.ToggleCollapsed(roots[0]); break;
    default:
      if (auto target = GetMoveTarget(command, roots[0])) Move(roots, target->parent, target->before);
      break;
  }
  return std::nullopt;
}
...
void TabOrganizationController::AlignTabOrder() {
  if (aligning_ || !dragged_.empty()) return;
  ...
  for (const auto& id : tree_.PageOrder()) {
    ...
    if (index != destination) tabs_->MoveWebContentsAt(index, destination, false, std::nullopt);
    ++destination;
  }
}
...
void TabOrganizationController::BeginTabDrag() {
  ...
  drop_parent_.reset();
  drop_before_.reset();
  if (dragged_.empty()) return;
  auto selection = tabs_->selection_model().GetListSelectionModel();
  for (const auto& id : tree_.PagesIn(dragged_)) {
    if (auto* tab = TabFor(id)) selection.AddIndexToSelection(tabs_->GetIndexOfTab(tab));
  }
  tabs_->SetSelectionFromModel(selection);
}

void TabOrganizationController::SetDropDestination(TreeId parent,
                                                    std::optional<TreeId> before) {
...
void TabOrganizationController::EndTabDrag(bool completed) {
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
      base::BindOnce(&TabOrganizationController::FinishTabDrag,
...
void TabOrganizationController::FinishTabDrag(bool completed) {
  ...
  if (completed && parent) {
    if (ids.empty()) ids = SelectionFor();  // A native drag from another window.
...
void TabOrganizationController::OnTabChangedAt(tabs::TabInterface*, TabChangeType) { Sync(); }
void TabOrganizationController::OnTabPinnedStateChanged(tabs::TabInterface*, int) { Sync(); }
void TabOrganizationController::TabGroupedStateChanged(TabStripModel*,
    std::optional<tab_groups::TabGroupId>, std::optional<tab_groups::TabGroupId>,
    tabs::TabInterface*, int) { Sync(); }
void TabOrganizationController::OnSplitTabChanged(const SplitTabChange&) { Sync(); }

void TabOrganizationController::ScheduleUpdate() {
  if (update_pending_) return;
  update_pending_ = true;
  base::SequencedTaskRunner::GetCurrentDefault()->PostTask(FROM_HERE,
      base::BindOnce(&TabOrganizationController::Update, weak_factory_.GetWeakPtr()));
}

void TabOrganizationController::Update() {
  update_pending_ = false;
  auto* service = window_->GetProfile()->IsOffTheRecord() ? nullptr
  ...
  for (const auto& [handle, id] : pages_) {
    ...
    tab->GetContents()->SetUserData(&kPageTreeStateKey, std::move(transient));
    auto encoded = base::WriteJson(state);
    if (service && encoded) {
      service->AddTabExtraData(window_->GetSessionID(),
          sessions::SessionTabHelper::IdForTab(tab->GetContents()), kSessionKey, *encoded);
    }
  }
  changed_.Notify();
}

base::WeakPtr<TabOrganizationController> TabOrganizationController::GetWeakPtr() {
  return weak_factory_.GetWeakPtr();
}

base::CallbackListSubscription TabOrganizationController::Observe(base::RepeatingClosure callback) {
