...
class TaskNameDelegate : public ui::DialogModelDelegate {
 public:
  TaskNameDelegate(base::WeakPtr<TabOrganizationController> controller, TreeId id)
      : controller_(controller), id_(std::move(id)) {}
  void Save() {
    if (controller_) controller_->Rename(id_, dialog_model()->GetTextfieldByUniqueId(kTaskNameField)->text());
  }
 private:
...
TabTreeAdapter::TabTreeAdapter(views::View* host, TabCollectionNode* collection)
  ...
  if (!collection->GetController()) return;
  auto* controller = TabOrganizationController::From(
      collection->GetController()->GetBrowserView()->browser());
  if (!controller) return;
  controller_ = controller->GetWeakPtr();
  organize_ = host_->AddChildView(std::make_unique<views::MenuButton>(
      base::BindRepeating([](base::WeakPtr<TabTreeAdapter> self, const ui::Event&) {
        if (self) self->OpenMenu(self->organize_->GetBoundsInScreen().bottom_left(),
                                 ui::mojom::MenuSourceType::kNone);
      }, weak_factory_.GetWeakPtr()), l10n_util::GetStringUTF16(IDS_UW_ORGANIZE)));
...
TabTreeAdapter::~TabTreeAdapter() {
  runner_.reset();
  name_dialog_.reset();
  for (const auto& [id, view] : tasks_) host_->RemoveChildViewT(view.get());
  for (const auto& [id, view] : disclosures_) host_->RemoveChildViewT(view.get());
  if (organize_) host_->RemoveChildViewT(organize_.get());
  if (scratchpad_) host_->RemoveChildViewT(scratchpad_.get());
}

void TabTreeAdapter::Refresh() {
  if (!controller_) return;
  const auto selection = controller_->SelectionFor();
  if (selection != last_tab_selection_) selected_task_.reset();
    ...
    if (node->value.kind == TabTree::Kind::kTask) {
      ...
      if (!tasks_.contains(id)) {
        auto button = std::make_unique<views::LabelButton>(
            base::BindRepeating(&TabTreeAdapter::SelectTask, weak_factory_.GetWeakPtr(), id), node->GetTitle());
      ...
      if (node->value.collapsed) tasks_.at(id)->GetViewAccessibility().SetIsCollapsed();
      else tasks_.at(id)->GetViewAccessibility().SetIsExpanded();
    }
    if ((node->value.kind == TabTree::Kind::kTask || node->value.kind == TabTree::Kind::kPage) &&
        !node->children().empty()) {
      parents.insert(id);
      if (!disclosures_.contains(id)) {
        disclosures_.emplace(id, host_->AddChildView(std::make_unique<views::LabelButton>(
            base::BindRepeating(&TabTreeAdapter::Toggle, weak_factory_.GetWeakPtr(), id), u"")));
      }
      ...
      button->GetViewAccessibility().SetName(l10n_util::GetStringUTF16(
          node->value.collapsed ? IDS_UW_EXPAND_BRANCH : IDS_UW_COLLAPSE_BRANCH));
...
views::ProposedLayout TabTreeAdapter::CalculateLayout(const views::SizeBounds& bounds) const {
  views::ProposedLayout layout;
  if (!controller_) return layout;
  rows_.clear();
  ...
  auto place = [&](views::View* view, bool visible, int x, int height) {
    gfx::Rect rect(x, y, std::max(0, width - x - kPadding), visible ? height : 0);
  ...
  for (const auto& child : collection_->children()) {
    if (auto* view = views::AsViewClass<TabView>(child->view())) {
      if (auto id = controller_->IdFor(view->GetTabInterface())) tab_views.emplace(*id, view);
    }
  }
  ...
  std::function<void(const TabTree::Node*, int, bool)> visit =
    ...
    const TreeId id = node->value.id;
    if (node->value.kind == TabTree::Kind::kScratchpad) row = scratchpad_;
    else if (auto task = tasks_.find(id); task != tasks_.end()) row = task->second;
    else if (auto tab = tab_views.find(id); tab != tab_views.end()) row = tab->second;
    ...
    if (row) {
      ...
      const int height = std::max(kHeaderHeight, row->GetPreferredSize(
          views::SizeBounds(std::max(0, width - x - kDisclosureWidth - kPadding), {})).height());
      const auto rect = place(row, visible,
          node->value.kind == TabTree::Kind::kScratchpad ? x : x + kDisclosureWidth, height);
      if (auto disclosure = disclosures_.find(id); disclosure != disclosures_.end()) {
        layout.child_layouts.emplace_back(disclosure->second.get(), visible,
            gfx::Rect(x, y, kDisclosureWidth, height));
        placed.insert(disclosure->second);
      }
      if (visible) {
    ...
    const int child_depth = node->value.kind == TabTree::Kind::kScratchpad ? 0 : depth + 1;
    for (const auto& child : node->children()) visit(child.get(), child_depth, visible && !node->value.collapsed);
  };
  for (const auto& node : tree.root()->children()) visit(node.get(), 0, true);
  ...
  for (auto* child : collection_->GetDirectChildren()) {
    ...
    const int height = child->GetPreferredSize(views::SizeBounds(width - 2 * kPadding, {})).height();
    place(child, true, kPadding, height);
    y += height + 2;
  }
  for (auto* child : host_->children()) {
    if (!placed.contains(child)) layout.child_layouts.emplace_back(child, false, gfx::Rect());
  }
...
std::vector<TreeId> TabTreeAdapter::Selection() const {
  if (!controller_) return {};
  if (selected_task_ && controller_->tree().Find(*selected_task_)) return {*selected_task_};
  return controller_->SelectionFor();
}

void TabTreeAdapter::SelectTask(TreeId id) {
  selected_task_ = id;
  if (auto it = tasks_.find(id); it != tasks_.end()) it->second->RequestFocus();
}

void TabTreeAdapter::Toggle(TreeId id) {
  if (controller_) controller_->tree().ToggleCollapsed(id);
}

void TabTreeAdapter::OpenMenu(const gfx::Point& point, ui::mojom::MenuSourceType source) {
  if (!controller_ || !host_->GetWidget()) return;
  menu_selection_ = Selection();
  runner_.reset();
  menu_ = std::make_unique<ui::SimpleMenuModel>(this);
  menu_->AddItem(TabOrganizationController::kRenameTask, l10n_util::GetStringUTF16(IDS_UW_RENAME_TASK));
  TabOrganizationController::AppendCommands(menu_.get());
  runner_ = std::make_unique<views::MenuRunner>(menu_.get(), views::MenuRunner::CONTEXT_MENU);
...
void TabTreeAdapter::ExecuteCommand(int command, int) {
  if (!controller_ || !controller_->CanExecute(command, menu_selection_)) return;
...
void TabTreeAdapter::EditName(TreeId task) {
  ...
  if (!node || node->value.kind != TabTree::Kind::kTask) return;
  ...
  auto model = ui::DialogModel::Builder(std::move(delegate))
      ...
      .AddTextfield(kTaskNameField, l10n_util::GetStringUTF16(IDS_UW_TASK_NAME), node->GetTitle())
      .AddOkButton(base::BindOnce(&TaskNameDelegate::Save, base::Unretained(raw_delegate)))
      .AddCancelButton(base::DoNothing())
      .Build();
  auto bubble = std::make_unique<views::BubbleDialogModelHost>(
      std::move(model), organize_, views::BubbleBorder::TOP_LEFT);
  name_dialog_ = views::BubbleDialogDelegate::CreateBubble(std::move(bubble));
  name_dialog_->Show();
}

void TabTreeAdapter::ShowContextMenuForViewImpl(views::View* source,
    const gfx::Point& point, ui::mojom::MenuSourceType type) {
  for (const auto& [id, view] : tasks_) {
    if (view == source) { selected_task_ = id; break; }
  }
  OpenMenu(point, type);
}

std::optional<TabTreeAdapter::Destination> TabTreeAdapter::DropDestination(const gfx::Point& point) const {
  if (!controller_) return std::nullopt;
  const auto& tree = controller_->tree();
  for (const auto& row : rows_) {
    if (point.y() > row.bounds.bottom()) continue;
    ...
    return Destination{node->parent()->value.id,
        next < siblings.size() ? std::make_optional(siblings[next]->value.id) : std::nullopt};
  }
...
void TabTreeAdapter::TrackNativeDrop(const gfx::Point& screen) {
  if (!controller_) return;
  auto target = DropDestination(views::View::ConvertPointFromScreen(host_, screen));
  if (target) {
    // Page tabs cannot be root siblings of Scratchpad/task sections.
    if (target->parent == controller_->tree().root()->value.id)
      target = Destination{controller_->tree().scratchpad()->value.id, std::nullopt};
    controller_->SetDropDestination(target->parent, target->before);
  }
}

std::optional<int> TabTreeAdapter::LinkDropIndex(const gfx::Point& point) const {
  if (!controller_) return std::nullopt;
  for (const auto& row : rows_) {
    if (point.y() <= row.bounds.bottom()) {
      if (auto* tab = controller_->TabFor(row.id))
        return tab->GetBrowserWindowInterface()->GetTabStripModel()->GetIndexOfTab(tab);
    }
  }
  return std::nullopt;
}
...
std::optional<TreeId> TabTreeAdapter::ReadDraggedTask(const ui::OSExchangeData& data) const {
  auto pickle = data.GetPickledData(DragFormat());
  if (!pickle || !controller_) return std::nullopt;
  std::string id;
  base::PickleIterator iterator(*pickle);
  if (!iterator.ReadString(&id)) return std::nullopt;
  const auto* node = controller_->tree().Find(TreeId(id));
  return node && node->value.kind == TabTree::Kind::kTask ? std::make_optional(TreeId(id)) : std::nullopt;
}
...
int TabTreeAdapter::DragUpdated(const ui::DropTargetEvent& event) {
  ...
  return id && target && controller_->tree().CanMove({*id}, target->parent)
      ? ui::DragDropTypes::DRAG_MOVE : ui::DragDropTypes::DRAG_NONE;
}

views::View::DropCallback TabTreeAdapter::DropCallback(const ui::DropTargetEvent& event) {
  auto id = ReadDraggedTask(event.data());
  auto target = DropDestination(event.location());
  if (!id || !target) return {};
  return base::BindOnce(&TabTreeAdapter::Drop, weak_factory_.GetWeakPtr(), *id, *target);
}

void TabTreeAdapter::Drop(TreeId id, Destination target, const ui::DropTargetEvent&,
...
void TabTreeAdapter::WriteDragDataForView(views::View* sender, const gfx::Point&, ui::OSExchangeData* data) {
  for (const auto& [id, view] : tasks_) {
...
int TabTreeAdapter::GetDragOperationsForView(views::View* sender, const gfx::Point&) {
  for (const auto& [id, view] : tasks_) {
    if (view == sender) return ui::DragDropTypes::DRAG_MOVE;
  }
  return ui::DragDropTypes::DRAG_NONE;
}

bool TabTreeAdapter::CanStartDragForView(views::View*, const gfx::Point& press, const gfx::Point& current) {
